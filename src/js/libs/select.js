...
data-one-select - селекты внутри оболочки с атрибутом будут отображаться только по одному
...
data-label для плейсхолдера, добавляет label в селект
data-show для плейсхолдера, показывает его в списке (только для единичного выбора)

Для элемента (option):
data-class="имя класу" - добавляет класс
